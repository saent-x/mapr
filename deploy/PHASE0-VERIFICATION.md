...
| (g) RAM/CPU under concurrent ingest + LLM | **OPERATOR (box)** — resource budget + `MAX_CONCURRENT_LLM=1` + off-peak ingest documented in deploy/README. | Run on the box; watch `docker stats` during a refresh + QA load. |
| (h) backup → restore drill | **OPERATOR (box)** — `deploy/backup/backup.sh` (pg_dump + convex export off-box) + `deploy/backup/restore.md`. | Run backup, then restore into a scratch deployment per restore.md. |
