...
- For geopolitical arc validation, flat-map arcs are rendered on the canvas and may not expose reliable hit targets to `agent-browser`; when direct clicks are flaky, use `agent-browser eval` to import the app stores/utilities, select a target arc in-state, and then review the rendered `ArcPanel` plus screenshots for the user-visible evidence.
